...
class HiddenMarkovModel:
    ...
    def __init__(self, observation_states: np.ndarray, hidden_states: np.ndarray, prior_p: np.ndarray, transition_p: np.ndarray, emission_p: np.ndarray):
        ...
        if len(prior_p) != N:
            raise ValueError(f"prior_p length {len(prior_p)} doesn't match number of hidden states {N}")

        if transition_p.shape != (N, N):
            raise ValueError(f"transition_p shape {transition_p.shape} doesn't match expected ({N}, {N})")

        if emission_p.shape != (N, M):
            raise ValueError(f"emission_p shape {emission_p.shape} doesn't match expected ({N}, {M})")

        if not np.isclose(np.sum(prior_p), 1.0):
            raise ValueError(f"prior_p doesn't sum to 1.0 (got {np.sum(prior_p)})")

        for i in range(N):
            if not np.isclose(np.sum(transition_p[i]), 1.0):
                raise ValueError(f"transition_p row {i} doesn't sum to 1.0 (got {np.sum(transition_p[i])})")
            if not np.isclose(np.sum(emission_p[i]), 1.0):
                raise ValueError(f"emission_p row {i} doesn't sum to 1.0 (got {np.sum(emission_p[i])})")
    ...
    def forward(self, input_observation_states: np.ndarray) -> float:
        ...
        # Step 2. Calculate probabilities (recursion)
        for t in range(1, T):
        ...
        # Step 3. Return final probability (sum of last row)
    ...
    def viterbi(self, decode_observation_states: np.ndarray) -> list:
        ...
        for t in range(1, T):
            ...
            for s in range(N):
                # find the previous state that maximizes the path probability
                max_prob = -1
                max_prev = 0
                for prev in range(N):
